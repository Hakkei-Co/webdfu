...
;(function () {
  ...
  function logInfo (msg) {
    console.log(
      '%c logInfo ⏰ ',
      'background:#6e6e6e; color: #cdfdce;, ⚛︎ logInfo ⚛︎ msg',
      msg
    )
  ...
  function logError (msg) {
    console.log(
      '%c logError ⏰ ',
      'background:#6e6e6e; color: #cdfdce;, ⚛︎ logError ⚛︎ msg',
      msg
    )
  ...
  document.addEventListener('DOMContentLoaded', event => {
    ...
    async function connect (device) {
      ...
      try {
        ...
        console.log(
          '%c DFU Functional Descriptor ⏰ ',
          'background:#6e6e6e; color: #cdfdce;, ⚛︎ connect ⚛︎ desc',
          desc
        )
      } catch (error) {
        onDisconnect(error)
      ...
      if (device) {
        ...
        navigator.usb
          ...
          .then(async selectedDevice => {
            ...
            console.log(
              '%c Interfaces ⏰ ',
              'background:#6e6e6e; color: #cdfdce;, ⚛︎ interfaces',
              interfaces
            )
    ...
    async function gracefullyResetDevice (device) {
      console.log('%c resetting stuff ⏰ ', 'background:#6e6e6e; color: #cdfdce;, ⚛︎ gracefullyResetDevice ⚛︎ device', device);

      try {
        const res = await device.reset()
        console.log('%c after reset ⏰ ', 'background:#6e6e6e; color: #cdfdce;, ⚛︎ gracefullyResetDevice ⚛︎ res', res);
